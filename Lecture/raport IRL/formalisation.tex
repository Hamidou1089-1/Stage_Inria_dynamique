...
\subsubsection*{Methode de generation}
...
\\
Par la définition du risque systémique, on cherche à savoir pour un choc donné, avec un niveau d'interconnexion connu
quel proportion de banque font défaut.
\\\\
Il nous manque un ingredient pour que cette simulation soit utile, comment mettre à jour le réseau et compter le nombre de defaut après un choc ?\\
...
Ainsi donc on définit une matrice R, qui va représenter la proportion de chaque dette:
...
Pour calculer ce vecteur d'équilibre, Eisenberg et Noe proposent un algorithme itératif, parfois appelé \textit{algorithme des défauts fictifs} \cite{eisenberg2001systemic}. Le principe est de construire une séquence de vecteurs de paiement qui converge vers la solution. On commence avec $P^0 = \bar{P}$, qui correspond au cas où toutes les banques remboursent intégralement leurs dettes. Puis on construit itérativement:
$$P^1 = \phi(P^0), P^2 = \phi(P^1), P^3 = \phi(P^2), \ldots$$

Eisenberg et Noe démontrent que cette séquence est monotone décroissante: $P^0 \geq P^1 \geq P^2 \geq \ldots$, ce qui signifie qu'à chaque itération, les paiements estimés diminuent ou restent constants. Comme cette séquence est bornée inférieurement par le vecteur nul (les paiements ne peuvent pas être négatifs), elle converge nécessairement vers une limite $P' = \lim_{k \to \infty} P^k$.
